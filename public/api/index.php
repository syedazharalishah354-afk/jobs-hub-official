...
if ($path === 'admin/applications' && $method === 'GET') {
    ...
    $rows = $stmt->fetchAll();
    $apps = array_map(function($r) {
        $r['fullName'] = $r['full_name'] ?? ($r['fullName'] ?? '');
        $r['candidateName'] = $r['full_name'] ?? ($r['fullName'] ?? '');
        $r['fatherName'] = $r['father_name'] ?? ($r['fatherName'] ?? '');
        $r['referenceNo'] = $r['reference_no'] ?? ($r['referenceNo'] ?? '');
        $r['rollNumber'] = $r['reference_no'] ?? ($r['referenceNo'] ?? '');
        $r['postalCode'] = $r['postal_code'] ?? ($r['postalCode'] ?? '');
        $r['jobPosition'] = $r['job_position'] ?? ($r['jobPosition'] ?? 'General Application');
        $r['jobTitle'] = $r['job_position'] ?? ($r['jobPosition'] ?? 'General Application');
        $r['jobCategory'] = $r['job_category'] ?? ($r['jobCategory'] ?? 'General');
        $r['cnicFrontUrl'] = $r['cnic_front_url'] ?? ($r['cnicFrontUrl'] ?? '');
        $r['cnicBackUrl'] = $r['cnic_back_url'] ?? ($r['cnicBackUrl'] ?? '');
        $r['applicantPhotoUrl'] = $r['applicant_photo_url'] ?? ($r['applicantPhotoUrl'] ?? null);
        $r['paymentScreenshotUrl'] = $r['payment_screenshot_url'] ?? ($r['paymentScreenshotUrl'] ?? null);
        $r['paymentScreenshot'] = $r['payment_screenshot_url'] ?? ($r['paymentScreenshotUrl'] ?? null);
        $r['paymentMethod'] = $r['payment_method'] ?? ($r['paymentMethod'] ?? null);
        $r['paymentTxnId'] = $r['payment_txn_id'] ?? ($r['paymentTxnId'] ?? null);
        $r['trxId'] = $r['payment_txn_id'] ?? ($r['paymentTxnId'] ?? null);
        $r['rejectionReason'] = $r['rejection_reason'] ?? ($r['rejectionReason'] ?? null);
        $r['mobile'] = $r['mobile'] ?? ($r['mobileNumber'] ?? '');
        $r['mobileNumber'] = $r['mobile'] ?? ($r['mobileNumber'] ?? '');
        $r['whatsapp'] = $r['whatsapp'] ?? ($r['mobile'] ?? '');
        $r['dob'] = $r['dob'] ?? '';
